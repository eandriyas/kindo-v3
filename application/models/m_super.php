...
class m_super extends CI_Model{
	...
	public function posted($limit, $offset){
		$sql = "SELECT kompetisi.id_kompetisi AS 'id_kompetisi',kompetisi.judul_kompetisi AS 'judul_kompetisi', main_kat.main_kat AS 'id_main_kat', sub_kat.sub_kat AS 'id_sub_kat', kompetisi.status AS 'status'
		FROM kompetisi INNER JOIN main_kat ON kompetisi.id_main_kat = main_kat.id_main_kat 
		INNER JOIN sub_kat ON kompetisi.id_sub_kat = sub_kat.id_sub_kat
		WHERE author = 1 AND status = 'posted' ORDER BY kompetisi.tgl_buat DESC LIMIT ".$limit." OFFSET ".$offset."";
	...
	public function draft($limit, $offset){
		...
		FROM kompetisi INNER JOIN main_kat ON kompetisi.id_main_kat = main_kat.id_main_kat 
		INNER JOIN sub_kat ON kompetisi.id_sub_kat = sub_kat.id_sub_kat
		WHERE author = 1 AND status = 'draft' ORDER BY kompetisi.tgl_buat DESC LIMIT ".$limit." OFFSET ".$offset."";
		$query = $this->db->query($sql);
		//struktur kendali untuk cek apakah data ada atau tidak
		if($query->num_rows() > 0) { //jika ada data yang ditemukan
			//maka menampilkan hasilnya sebagai baris array
