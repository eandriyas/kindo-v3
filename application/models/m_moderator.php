...
class m_moderator extends CI_Model{
	...
	function show_active_user($limit, $offset){
		$sql = "SELECT * FROM user WHERE status = 'active' AND level = 'moderator' LIMIT ".$limit." OFFSET ".$offset."";
	...
	function count_active_user(){
		$sql = "SELECT COUNT(*) AS 'total' FROM user WHERE status = 'active' AND level = 'moderator'";
		$query = $this->db->query($sql);
		if($query->num_rows() > 0) {
	...
	function show_banned_user($limit, $offset){
		$sql = "SELECT * FROM user WHERE status = 'banned' AND level = 'moderator' LIMIT ".$limit." OFFSET ".$offset."";
		$query = $this->db->query($sql);
		if($query->num_rows() > 0) {
			return $query->result_array();
		} else {
	...
	function count_banned_user(){
		$sql = "SELECT COUNT(*) AS 'total' FROM user WHERE status = 'banned' AND level = 'moderator'";
		$query = $this->db->query($sql);
		if($query->num_rows() > 0) {
			return $query->num_rows();
		}  else {
