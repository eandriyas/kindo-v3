	...
	<div class="container">
		<?php if ( 'on' == get_option('evolution_display_about_page') ) { ?>
			...
			?>
					<div id="about">
						...
						<?php the_content(''); ?>
						<a href="<?php the_permalink(); ?>" class="readmore"><span><?php esc_html_e('Baca Selengkapnya'); ?></span></a>
					</div> <!-- end #about -->
			<?php endwhile; wp_reset_postdata(); ?>
		<?php } ?>
		...
		<?php if ( 'on' == get_option('evolution_display_recent_blog_posts') ) { ?>
			<div id="recent-posts">
				...
				<?php
					$home_category_id = (int) get_cat_ID( get_option('evolution_home_recentblog_section') );
					
					while ( have_posts() ) : the_post();
						$width = apply_filters( 'evolution_home_blog_width', 60 );
						$height = apply_filters( 'evolution_home_blog_height', 60 );
						$titletext = get_the_title();
				...
				<a href="<?php echo esc_url( get_category_link( $home_category_id ) ); ?>" class="readmore"><span><?php esc_html_e('Baca Selengkapnya'); ?></span></a>
