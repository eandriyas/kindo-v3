...
if (!class_exists('TW_Pagination')) {
	class TW_Pagination {
		...
		var $optionsName = 'tw_pagination_options';
		//	@var string $localizationDomain (Domain used for localization)
		var $localizationDomain = 'tw-pagination';
		...
		function get_options() {
			if (! $options = get_option ( $this->optionsName )) {
				$options = array (
					'title' => __ ( 'Pages:', $this->localizationDomain ), 
					'nextpage' => '&raquo;', 
					'previouspage' => '&laquo;', 
					'css' => true, 
...
<div class="wrap">
	...
	<?php wp_nonce_field('tw-pagination-update-options'); ?>	
		...
		<table class="form-table">
			...
			<tr valign="top">
				<th scope="row"><?php _e('TW-Pagination CSS File:', $this->localizationDomain); ?></th>
				<td><label for="css">
					<input type="checkbox" id="css" name="css" <?php echo ($this->options['css'] === true) ? "checked='checked'" : ""; ?>/> <?php printf(__('Include the default stylesheet tw-pagination.css? TW-Pagination will first look for <code>tw-pagination.css</code> in your theme directory (<code>themes/%s</code>).', $this->localizationDomain), get_template()); ?></label></td>
			</tr>
			<tr valign="top">
		...
		<p>&nbsp;</p>
		<h3><?php _e('Implement', $this->localizationDomain); ?></h3>
		<p><strong><?php _e('For posts pagination:', $this->localizationDomain)?></strong></p>
		<p><?php printf(__('1. Open the theme files where you\'d like pagination to be used. Usually this is the <code>themes/%s/loop.php</code> file.', $this->localizationDomain), get_template()); ?></p>
		<p><?php _e('2. Replace your existing <code>previous_posts_link()</code> and <code>next_posts_link()</code> code block with the following:', $this->localizationDomain); ?></p>
		<p><code>&lt;?php if(function_exists(&#39;tw_pagination&#39;)) tw_pagination(); ?&gt;</code></p>
		<p><strong><?php _e('For comments pagination:', $this->localizationDomain)?></strong><p>
		<p><?php printf(__('1. Open the theme file(s) where you\'d like comments pagination to be used. Usually this is the <code>themes/%s/comments.php</code> file.', $this->localizationDomain), get_template()); ?></p>
		<p><?php _e('2. Replace your existing <code>previous_comments_link()</code> and <code>next_comments_link()</code> code block with the following:', $this->localizationDomain); ?></p>
		<p><code>&lt;?php if(function_exists(&#39;tw_pagination_comments&#39;)) tw_pagination_comments(); ?&gt;</code></p>
		<p><strong><?php _e('Note:', $this->localizationDomain)?></strong></p>
		<p><?php _e('If you want to initialise new custom <code>WP_Query</code> object and want to paginate your query, than you should call <code>tw_pagination()</code> function by passing your instance as first argument.', $this->localizationDomain)?></p>
		<p><?php _e('For example, if you initialise new query like this <code>&lt;?php $the_query = new WP_Query( $args ); ?&gt;</code> than you should call <code>tw_pagination()</code> function like this: <code>&lt;?php if(function_exists(&#39;tw_pagination&#39;)) tw_pagination($the_query); ?&gt;</code>', $this->localizationDomain)?></p>
